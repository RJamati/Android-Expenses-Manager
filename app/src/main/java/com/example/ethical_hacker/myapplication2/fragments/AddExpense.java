...
public class AddExpense extends Fragment implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.exp_submit) {
            boolean inserted = db.setExpense(sessionManager.get_User_Id(), categoryspinner.getSelectedItem().toString(), Integer.valueOf(amount.getText().toString()), comment.getText().toString(), edittext.getText().toString());
            if (inserted) {
                Toast.makeText(getActivity(), "Expense has been added", Toast.LENGTH_LONG).show();
                clearText();
            }
        }
    }
